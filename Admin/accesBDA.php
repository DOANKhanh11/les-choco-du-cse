        ...
        public function modifyElement($tableauDeLElement) {
            try {
                ...
                if ($element[0]=='produit') {
                    ...
                    $element = $tableauDeLElement;
                    $requete = $this->conn->prepare("UPDATE produit SET referenceProduit = ?, dateCreationProduit = ?, nomProduit = ?, stockProduit = ?, statutProduit = ?, nbrPointProduit = ?, descriptionProduit = ?, valeurNutriProduit = ?, idDeFournisseur = ? WHERE idProduit = ?;");
                    $requete->bindValue(1,$element[3]);
                    $requete->bindValue(2,$element[2]);
...
// Méthode pour vérifier si une carte existe déjà dans la base de données pour une entreprise donnée
        public function entrepriseCarteExist($codeCarte, $entreprise)
        {
            //la rêquete SQL pour vérifier l'existence de la carte dans la base de données pour l'entreprise spécifiée
            $query = "SELECT COUNT(*) AS count FROM carte WHERE codeCarte = :codeCarte and entreprise = :entreprise";
            $stmt = $this->conn->prepare($query); 
            // Préparer la requête
            $stmt = $this->conn->prepare($query);
        
            // Binder les paramètres
            $stmt->bindParam(':codeCarte', $codeCarte);
