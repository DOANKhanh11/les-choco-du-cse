<?php
    class accesBD
    {
        ...
        public function __construct()
        {
            ...
            /*$this->hote = 'joyeuxcsqhbd.mysql.db';
        ...
        public function totalVueProduit($codePin) 
            {
                try {
                    ...
                    if ($pointCarte <= 0) {
                        $requete=$this->conn->prepare('select count(nomProduit) as totalP from produit inner join lesImages on lesImages.idImage = produit.idProduit where principaleImage = 1 and nbrPointProduit <= ? and produit.statutProduit = 1;'); //requête utilisée en ligne : SELECT count(nomProduit) AS totalP from produit_accueil, carte, entreprise where entreprise.idEntreprise = carte.idDeEntreprise and principaleImage = 1 and nbrPointProduit <= nbrPointEntreprise and codePin like ?;');
                        $requete->bindValue(1,$this->selectPointEntrepriseWhereCodePin($codePin)); //utilisé en ligne : $requete->bindValue(1,$codePin);
                        $requete->execute();
                        $retour = '';
                        ...
                        {
                            echo $row;
                            $retour = $row->totalP.'<br>';
                        }
                    }
                    else {
                        $requete=$this->conn->prepare('select count(idProduit) as totalP from produit'./* inner join lesImages on produit.idProduit = lesImages.idImage*/' where './*lesImages.principaleImage = 1 and */'produit.nbrPointProduit <= ? and produit.statutProduit = 1;');//requête utilisée en ligne : SELECT count(nomProduit) AS totalP from produit_accueil, carte where principaleImage = 1 and nbrPointProduit <= nbrPointCarte and codePin like ?;');
                    ...
                    //
                    // $requete=$this->conn->prepare('SELECT count(nomProduit) AS totalP from produit_accueil, carte where principaleImage = 1 and nbrPointProduit <= nbrPointCarte and codePin like ?;');
                    // $requete->bindValue(1,$codePin);
                    // $requete->execute();
                    // $retour = '';
                    // while ($row=$requete->fetch(PDO::FETCH_OBJ))
